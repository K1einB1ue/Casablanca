...
public class PlayerInput : InputBase
{
    ...
    public override void FixedUpdate() {
        ...
        if (Input.GetKey(KeyCode.Alpha1))               { this.K1Event                              ?.Invoke(); this.K_Event.Invoke(1); }
        if (Input.GetKey(KeyCode.Alpha2))               { this.K2Event                              ?.Invoke(); this.K_Event.Invoke(2); }
        if (Input.GetKey(KeyCode.Alpha3))               { this.K3Event                              ?.Invoke(); this.K_Event.Invoke(3); }
        if (Input.GetKey(KeyCode.Alpha4))               { this.K4Event                              ?.Invoke(); this.K_Event.Invoke(4); }
        if (Input.GetKey(KeyCode.Alpha5))               { this.K5Event                              ?.Invoke(); this.K_Event.Invoke(5); }
        if (Input.GetKey(KeyCode.Alpha6))               { this.K6Event                              ?.Invoke(); this.K_Event.Invoke(6); }
        if (Input.GetKey(KeyCode.Alpha7))               { this.K7Event                              ?.Invoke(); this.K_Event.Invoke(7); }
